...
def supplier_view(request):
	...
	size = len(Product.objects.filter(flintsupplierid= currentUser.id))
	print(size)
	output = "products"
	if size == 1:
	...
	context ={
		'user_content':  Product.objects.filter(flintsupplierid= currentUser.id),
		'num_products' :output,
	...
	if pform.is_valid():
		...
		new_product = Product(pid=ids,name=item,Brand_Name=brand,
			description=description,origin=origin,price=price,image= pic,stock=stock,Unit=unit,flintsupplierid=currentUser, Packaging=pack,Active=True)
		new_product.save()
